...
export function processData(raw) {
  ...
  if (gac && gac.rows) gac.rows.forEach(r => { gSpesa += parseFloat(r[8]||0); });
  if (ma && ma.rows) ma.rows.forEach(r => {
    if (typeof r[0]==='string' && r[0].indexOf('In attesa')>=0) return;
  ...
  if (gac && gac.rows) gac.rows.forEach(row => {
    ...
    googleCampaigns.push({
      ...
      type: row[1]||'', roas: parseFloat(row[12]||0),
      spend: parseFloat(row[8]||0), ctr: parseFloat(row[6]||0),
      cpc: parseFloat(row[7]||0), conv: parseFloat(row[9]||0),
  ...
  if (ma && ma.rows) ma.rows.forEach(row => {
    ...
    metaCampaigns.push({
      ...
      type: row[3]||'', roas: parseFloat(row[14]||0),
      spend: parseFloat(row[10]||0), ctr: parseFloat(row[8]||0),
      cpc: parseFloat(row[9]||0), conv: parseFloat(row[11]||0),
  ...
  return {
    kpi: {
      spesa: totSpesa, roas: avgRoas, conv: totConv,
      ctr: avgCtr, click: totClick, cpc: avgCpc,
      prevSpesa, prevRoas, prevConv, prevCtr, prevClick, prevCpc
    },
    platform: { gPct, mPct, gSpesa, mSpesa },
    markets, dailySpend, googleCampaigns, metaCampaigns,
    budget, alerts
  };
}
